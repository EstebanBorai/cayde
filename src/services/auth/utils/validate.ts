 ...
 */
const isValidPassword = (password: string): boolean => new RegExp(/^(?=.*[0-9])(?=.*[a-z])(?=.*[A-Z])(?=.*[*.!@$%^&(){}[]:;<>,.?\/~_+-=|\]).{8,32}$/).test(password);
 ...
 */
const isValidEmail = (email: string): boolean => new RegExp(/^[\w-.]+@([\w-]+\.)+[\w-]{2,4}$/).test(email);
 ...
 */
const isValidUserName = (username: string): boolean => new RegExp(/^[A-Za-z0-9]{8,16}/).test(username);
